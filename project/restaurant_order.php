					...
					<div id="ext_upd" style="display: none;" class='show'></div>
                        <section  id="banner">
							...
							<?php
                                ...
                                $select = "SELECT * FROM `master_list` as mas left join `restaurant` as res on mas.res_id = res.res_id where mas.state != 3 and mas.state != 4";
                                if($name!=""){
                                    $select.=" and `email` LIKE '%".$name."%'";
                                }
                                ...
                                if($num=='0'){
                                    ...
                                    While($row=mysqli_fetch_assoc($ret)){
                                        echo"<tr>
											...
											echo"</td>
											<td>
												<input type='button' class='btn btn-outline-warning btn-lg' value='更改訂單狀態' onclick='upd($row[mas_id]);'></input>
											</td>
											<td>
