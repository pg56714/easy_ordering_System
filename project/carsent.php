...
include"connect.php" ;
  ...
  //存進訂單主表
  $restaurant1 ="INSERT INTO `master_list`(`mas_id`, `res_id`, `email`, `date`, `time`, `state`) VALUES ('$masnum','0','$email','$date','$time','0')";
  $restaurant2 = mysqli_query($conn, $restaurant1);

//存進訂單明細
//$booking1 ="INSERT INTO `booking`(`shop_id`, `mas_id`, `amount`) VALUES ('[value-2]','$masnum','[value-4]')";
 
    reset($_POST["shop_id"]);  
    reset($_POST["amount"]); 

    while ((list($k1, $v) = each($_POST["shop_id"])) && (list($k2, $z) = each($_POST["amount"]))) {

    $booking1 ="INSERT INTO `booking`(`shop_id`, `mas_id`, `amount`) VALUES ('$v','$masnum','$z')";
	
	$booking2 = mysqli_query($conn, $booking1);}


   if($restaurant2 == true ){
	 if($booking2 == true ){
	   $delete = "DELETE FROM `car` WHERE `email` = '$email' ";
       $delete2 = mysqli_query($conn, $delete);
	   echo"<Script>swal('已送出訂單，請查看訂單明細','','success')</Script>";
	   echo '<meta http-equiv=REFRESH CONTENT=1;url=Product.php>'; 
       }
	  }
   else{echo"<Script>swal('訂單送出失敗，請稍後再試','','error')</Script>";echo '<meta http-equiv=REFRESH CONTENT=1;url=Product.php>'; }
